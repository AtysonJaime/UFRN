...
def inverse_kinematic(theta_in_rads: float, velocity: numpy.ndarray) -> PioneerWheelVelocity:
    """
    ...
    linear_velocity = velocity[0] * numpy.cos(theta_in_rads) + velocity[1] * numpy.sin(theta_in_rads)
...
def direct_kinematic(left_wheel_in_rads: float, right_wheel_in_rads: float, theta_in_rads: float) -> PioneerVelocity:
    ...
    delta_x = numpy.cos(theta_in_rads) * mean
    delta_y = numpy.sin(theta_in_rads) * mean
