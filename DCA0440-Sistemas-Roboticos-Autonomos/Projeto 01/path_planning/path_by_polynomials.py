...
def is_in_undefined_region(angle_in_degree: float) -> bool:
    return numpy.pi / 2 - DELTA <= angle_in_degree <= numpy.pi / 2 + DELTA
...
def find_coefficients(initial_position: Position, final_position: Position) -> tuple[list[float], list[float]]:
    ...
    d_i = numpy.tan(initial_position.theta_in_rads)
    d_f = numpy.tan(final_position.theta_in_rads)
...
def calculate_path_length(x_coefficients: list[float], y_coefficients: list[float]) -> float:
    ...
    def dl(lam) -> float:
        return numpy.sqrt(dx(lam) ** 2 + dy(lam) ** 2)

    t = numpy.linspace(0, 1, num=1000)
    length = numpy.trapz(dl(t), t)
...
def path_points_generator(p_x, p_y) -> list[float]:
    lamb = numpy.arange(0.0, 1.0, 0.001)
    positions = []
    for value in lamb:
